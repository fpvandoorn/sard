...
lemma locally_lipschitz_image_of_null_set_is_null_set_open [SigmaCompactSpace X] {d : ℕ}
    {f : X → Y} {W : Set X} (hW : IsClosed W) (hf : LocallyLipschitz (W.restrict f))
    {s : Set X} (hsu : s ⊆ W) (hs : μH[d] s = 0) : μH[d] (f '' s) = 0 := by
  ...
  suffices ass : ∀ n : ℕ, μH[d] (f '' (s ∩ W ∩ K n)) = 0 by sorry
    --have : f '' (s ∩ W ∩ K n) = f '' (s ∩ K n) := sorry
    --rewrite [← this] --- fold into the above?
  ...
  intro n
  -- Consider the cover (t_x) of W ∩ K_n induced by hf: for each x ∈ W,
  -- choose a neighbourhood t_x of x and a constant K_x such that f is K_x-Lipschitz on t_x.
  ...
  -- By construction, (U_x) is an open covering of W ∩ K_n.
  --have hcovering : K n ⊆ ⋃ (x : X), U x := fun y _ ↦ mem_iUnion_of_mem y (hxU y)
  have hcovering : W ∩ K n ⊆ ⋃ (x : W), U x := by
    intro y hy
    sorry --aesop-- let sdf := hxU y
    -- apply [mem_iUnion_of_mem y]
    --exact?
    --fun y hy ↦ mem_iUnion_of_mem y (hxU y)
  --#exit
  -- As W is closed, W ∩ K_n is also compact. Hence, (U_x) has a finite subcover V_1, ..., V_l.
  have : IsCompact (W ∩ K n) := (hcompact n).inter_left hW
  have cov' : W ∩ K n ⊆ ⋃ (i : X) (_ : i ∈ ↑W), U i := sorry

  let ⟨v, hv⟩ := IsCompact.elim_finite_subcover this U hUopen cov'
  #exit
